...
CREATE FUNCTION Fn_GetTopSellingProductPerType()
RETURNS TABLE
AS
RETURN
(
    WITH TotalSales AS (
        SELECT 
            ...
            tp.IdTypeProduct,
            tp.NameTypeProduct,
            SUM(db.QuantityProduct) AS TotalQuantitySold
        FROM 
            Products p
        JOIN 
            TypeProducts tp ON p.IdTypeProduct = tp.IdTypeProduct
        JOIN 
            DetailBills db ON p.IdProduct = db.IdProduct
        JOIN 
            Bills b ON db.IdBill = b.IdBill
        WHERE 
            b.TypeBill = N'Hóa Đơn Xuất Hàng'
        GROUP BY 
            p.IdProduct, p.NameProduct, tp.IdTypeProduct, tp.NameTypeProduct
    )
    SELECT 
        ts.IdTypeProduct,
        ts.NameTypeProduct,
    ...
    JOIN 
        (
            ...
            GROUP BY 
                IdTypeProduct
        ) AS MaxSales ON ts.IdTypeProduct = MaxSales.IdTypeProduct 
                       AND ts.TotalQuantitySold = MaxSales.MaxQuantity
);